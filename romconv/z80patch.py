...
def unpack_z80(name):
    with zipfile.ZipFile(name, 'r') as zip:
        ...
        # I sub-opcode dopo i prefissi CB/ED/DD/FD sono fetch M1: devono
        # passare dall'opZ80 della macchina, necessario per le CPU con
        # opcode cifrati (es. Pengo, Sega 315-5010). I byte di DD CB /
        # FD CB (CyclesXXCB) restano letture dati via RdZ80, come in MAME.
        for tbl in (b"CyclesCB", b"CyclesED", b"CyclesXX"):
            old = b"I=OpZ80(R->PC.W++);\r\n  R->ICount-=" + tbl + b"[I];"
            new = (b"I=OpZ80_INL(R->PC.W++); /* fetch M1: decodifica opcode (segacrpt) */\r\n"
                   b"  R->ICount-=" + tbl + b"[I];")
            assert code.count(old) == (2 if tbl == b"CyclesXX" else 1)
