...
def create_c_array(job_description, infiles, outfile, array_name):
    ...
    with open(outfile, "w") as f:
        f.write(f"// File generato da cpu_conv_jpn.py\n")
        f.write(f"// ROM per: {job_description}\n")
        f.write(f"// Dimensione totale: {len(all_rom_data)} bytes\n\n")

        # Set non cifrato: opcode e dati coincidono, ma il sorgente si
        # aspetta entrambi gli array (vedi pengo::opZ80 / pengo::rdZ80).
        for name in (array_name, array_name + "_op"):
