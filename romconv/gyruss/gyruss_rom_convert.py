...
def main():
    ...
    # "i8039" come argomento: genera SOLO gyruss_rom_i8039.h (drums MCU),
    # senza rigenerare gli altri header gia' validati
    if "i8039" in sys.argv[1:]:
        rom = load_file("gyrussk.3a")
        if rom is None:
            print("ERRORE: gyrussk.3a non trovato")
            return
        path = os.path.join(OUT_DIR, "gyruss_rom_i8039.h")
        write_i8039_rom_h(rom, path)
        print(f"  Written: {path} ({len(rom)} bytes)")
        return
    ...
    if i8039_rom is None:
        print("ERRORE: Impossibile caricare gyrussk.3a")
        return
    path = os.path.join(OUT_DIR, "gyruss_rom_i8039.h")
