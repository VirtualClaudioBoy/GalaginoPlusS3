  ...
  if (!state->inInterrupt) {
    ...
    if (!state->notINT && state->externalInterruptsEnabled) {
      ...
      if (state->irq_oneshot)
        state->notINT = true;  // HOLD_LINE: auto-clear all'acknowledge (solo gyruss)
...
void i8048_step(struct i8048_state_S *state) {
  ...
  case 0x86: // JNI addr
    // Gyruss uses an edge/one-shot IRQ and expects the active-low INT
    // polarity used by the Spinnerino core. Preserve the historical path
    // for DK/DK Jr., whose level-driven sound ROMs were tuned against it.
    cjump(state, state->irq_oneshot ? !state->notINT : state->notINT);
    break;
