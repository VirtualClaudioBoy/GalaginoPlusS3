...
int m6809_step(m6809_state *s, char count) {
  ...
  for (int i=0; i < count; i++) {
    ...
    /* Sample pending IRQ/FIRQ at every instruction boundary.  A request can
       arrive while masked and must fire as soon as ANDCC/RTI unmasks it. */
    check_interrupts(s);

    /* Both SYNC and CWAI stop fetching until check_interrupts releases or
       vectors the CPU. */
    if (s->halted) {
        s->cycles += 1;
        s->total_cycles += 1;
