...
static uint8_t mem_read(m6803_state *s, uint16_t addr) {
    if (s->is_6802) {
        if (addr < 0x0080) return m6803_internal_ram[addr];
        return m6803_ext_read_fn ? m6803_ext_read_fn(addr) : 0xFF;
    }
...
static void mem_write(m6803_state *s, uint16_t addr, uint8_t val) {
    if (s->is_6802) {
        if (addr < 0x0080) m6803_internal_ram[addr] = val;
        else if (m6803_ext_write_fn) m6803_ext_write_fn(addr, val);
        return;
    }
