...
public:
	...
	// persistent high scores. The machineType guard keeps the clones that
	// derive from this class (mspacman, crush, eyes, ...) from inheriting
	// pacman's NVS key: each needs its own key and hiscore.dat regions.
	const char *hiscoreKey() override { return machineType() == MCH_PACMAN ? "pacman" : 0; }
