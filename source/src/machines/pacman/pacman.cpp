...
// from MAME hiscore.dat: high score in BCD plus its on-screen copies in
// video RAM. The video RAM entries double as init detection: they only
// hold '0' digits (0x40) once the game has drawn the score line.
const hiscore_region_S *pacman::hiscoreRegions(unsigned char *count) {
  static const hiscore_region_S regions[] = {
    { 0x4e88, 4, 0x00, 0x00 },   // high score (BCD)
    { 0x43ed, 6, 0x40, 0x40 },   // video RAM: displayed high score digits
    { 0x43d1, 1, 0x48, 0x48 },   // video RAM
  };
  *count = 3;
  return regions;
}
