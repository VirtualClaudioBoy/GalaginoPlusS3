...
public:
	...
	// (main+audio), che a fine frame resta sempre a 1 (ultima riga di
	// run_frame) -> accesso diretto alle 2 regioni RAM (0x8000-0x8FFF e
	// 0x9000-0x93FF, offset diversi), bypassando il dispatch.
	const char *hiscoreKey() override { return "bombjack"; }
	const hiscore_region_S *hiscoreRegions(unsigned char *count) override;
	unsigned char hiscoreRead(unsigned short addr) override {
		return (addr <= 0x8FFF) ? memory[addr - 0x8000] : memory[addr - 0x9000 + 0x1000];
	}
	void hiscoreWrite(unsigned short addr, unsigned char value) override {
		if (addr <= 0x8FFF) memory[addr - 0x8000] = value;
		else                memory[addr - 0x9000 + 0x1000] = value;
	}
