...
public:
	...
	// (main+audio), che a fine frame resta a 1 a meno che nmi_enable sia
	// attivo -> accesso diretto al Work RAM (0xA800-0xAFFF), bypassando
	// il dispatch.
	const char *hiscoreKey() override { return "timeplt"; }
	const hiscore_region_S *hiscoreRegions(unsigned char *count) override;
	unsigned char hiscoreRead(unsigned short addr) override { return memory[MEM_WORKRAM + (addr - 0xA800)]; }
	void hiscoreWrite(unsigned short addr, unsigned char value) override { memory[MEM_WORKRAM + (addr - 0xA800)] = value; }
