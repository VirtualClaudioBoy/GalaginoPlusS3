...
// from MAME hiscore.dat: best-5 table with initials plus the high score
// shown in video RAM. The end bytes only match once the game has
// initialized the table, then the saved scores can be injected.
const hiscore_region_S *galaga::hiscoreRegions(unsigned char *count) {
  static const hiscore_region_S regions[] = {
    { 0x8a20, 0x2d, 0x00, 0x18 },   // best-5 scores + initials
    { 0x83ed, 6,    0x00, 0x24 },   // video RAM: displayed high score
  };
  *count = 2;
  return regions;
}
