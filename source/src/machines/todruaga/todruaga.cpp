...
void todruaga::reset() {
  ...
  if (!rom_cached) {
    ...
    if (rm && rs && gx) {
      ...
      rom_cached = true;
      Serial.println(F("[TODRUAGA] ROM+gfx copiate in DRAM (~47 KB)"));
    } else {
      if (rm) heap_caps_free(rm);
      if (rs) heap_caps_free(rs);
      if (gx) heap_caps_free(gx);
      Serial.println(F("[TODRUAGA] alloc DRAM fallita: ROM/gfx dalla flash"));
    }
  }
...
// ============================================================================
// High score (MAME hiscore.dat: todruaga/todruagao/todruagas)
// ============================================================================

const hiscore_region_S *todruaga::hiscoreRegions(unsigned char *count) {
  static const hiscore_region_S regions[] = {
    { 0x102a, 0x32, 0x00, 0x45 },   // tabella top score con iniziali
    { 0x100b, 0x03, 0x00, 0x00 },   // high score corrente
  };
