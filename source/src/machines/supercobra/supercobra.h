...
public:
  ...
  // High score persistente (NVS). Eredita run_frame() da scramble, stesso
  // rischio current_cpu (resta a 1 a meno che irq_enable[0] sia attivo) ->
  // accesso diretto alla RAM principale (0x8000-0x87FF), bypassando il
  // dispatch.
  const char *hiscoreKey() override { return "scobra"; }
  const hiscore_region_S *hiscoreRegions(unsigned char *count) override;
  unsigned char hiscoreRead(unsigned short addr) override { return memory[addr - CPU1_RAM_ADDR]; }
  void hiscoreWrite(unsigned short addr, unsigned char value) override { memory[addr - CPU1_RAM_ADDR] = value; }
