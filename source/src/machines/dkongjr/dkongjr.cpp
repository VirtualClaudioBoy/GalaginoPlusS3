...
// High score: da MAME hiscore.dat, voce "dkongjrj" (gruppo dkingjr/dkongjrb/
// dkongjrj/jrking/maguila) -- le nostre ROM sono il set GIAPPONESE
// (romszip/dkongjrj.zip, c_5ba.bin/c_5ca.bin/c_5ea.bin nel converter), NON il
// set US "dkongjr", che ha valori di init diversi (60b8=50/00, celle video
// 00/07/06/05 = default 007650 invece di 011800). Con i valori US il pattern
// non combaciava mai e il manager restava in WAIT_INIT per sempre (bug "hiscore
// non funziona" del primo test HW). Valori verificati con emulazione Python
// del boot (vedi project_alibaba per il metodo, harness sim_dkongjr.py).
const hiscore_region_S *dkongjr::hiscoreRegions(unsigned char *count) {
  static const hiscore_region_S regions[] = {
    { 0x6105, 0x9e, 0x10, 0x10 },
    { 0x60b8, 0x03, 0x00, 0x01 },
    { 0x7661, 0x01, 0x10, 0x10 },
    { 0x7641, 0x01, 0x00, 0x00 },
    { 0x7621, 0x01, 0x01, 0x01 },
    { 0x7601, 0x01, 0x01, 0x01 },
    { 0x75e1, 0x01, 0x08, 0x08 },
    { 0x75c1, 0x01, 0x00, 0x00 },
