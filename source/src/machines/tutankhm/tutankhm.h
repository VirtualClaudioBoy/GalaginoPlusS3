...
public:
    ...
    // High score persistente (NVS). CPU principale e' un MC6809 (non Z80):
    // il default hiscoreRead/Write (rdZ80/wrZ80) servirebbe lo Z80 AUDIO
    // separato, indirizzi sbagliati -> accesso diretto al Work RAM
    // (mappa 0x8800-0x8FFF -> memory[TUT_WORKRAM+...], RAM pura senza
    // side effect, verificato in m6809_read/write).
    const char *hiscoreKey() override { return "tutankhm"; }
    const hiscore_region_S *hiscoreRegions(unsigned char *count) override;
    unsigned char hiscoreRead(unsigned short addr) override { return memory[TUT_WORKRAM + (addr - 0x8800)]; }
    void hiscoreWrite(unsigned short addr, unsigned char value) override { memory[TUT_WORKRAM + (addr - 0x8800)] = value; }
