...
unsigned char pengo::opZ80(unsigned short Addr) {
  // Fetch M1: opcode decifrati con la tabella OPCODE della Sega 315-5010
  if (Addr < 0x8000)
    return pengo_rom_op[Addr];
  else
    return memory[Addr - 0x8000];
}

// hiscore regions: addresses from MAME hiscore.dat, but our ROM set
// (pengoja) initializes them with the values of the "penta" entry —
// verified on hardware via HISCORE_DEBUG serial dump (2026-07-03)
const hiscore_region_S *pengo::hiscoreRegions(unsigned char *count) {
  static const hiscore_region_S regions[] = {
    { 0x8840, 0x1e, 0xd0, 0x41 },   // high score table with initials
    { 0x880c, 2,    0xd0, 0x07 },   // top score
  };
  *count = 2;
  return regions;
}
