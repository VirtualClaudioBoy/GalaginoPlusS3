...
unsigned char *memory;
  ...
  // WHITELIST autofire: SOLO i giochi elencati qui hanno l'autofire attivo
  // (shooter dove il fuoco ripetuto e' utile). Tutti gli altri giochi
  // partono con autofire disattivato (fuoco a livello, es. i flipper
  // di pbaction). Per abilitare l'autofire su un altro gioco basta
  // aggiungere qui il suo MCH_*.
  static bool machineHasAutofire(machineBase *m) {
...
void setup() {
  ...
  // Maximum strip width is 240; existing machines still use only 224 pixels.
...
#ifdef MARQUEE_ENABLED
  // Show the marquee of the currently selected (menu) / running (game) machine
  {
    signed char mIdx = isMenu ? menu.machineIndexPreselection()
                              : menu.machineIndexSelected();
    if (mIdx >= 0)
      marquee.select((unsigned char)machines[mIdx]->machineType());
  }
#endif
#ifdef MASTER_ATTRACT_MUTE_AUDIO
  // MASTER_ATTRACT_MUTE_AUDIO: durante una demo lanciata dal master attract
  // l'audio resta muto per tutta la demo. Un input generico (joystick,
  // volume) NON lo riattiva: il mute viene rimosso solo quando l'utente
  // avvia davvero la partita (COIN/START -> startOrCoinPressed) o seleziona
  // una macchina dal menu (menu.handle() mette attractPlay = false).
  // NB: il flag va consumato OGNI frame (non solo quando isAttractPlay()):
  // se venisse letto solo con lo short-circuit, una pressione di START/COIN
  // fatta nel menu (selezione/crediti) lascerebbe userPlayIntent=1 in sospeso
  // e la demo del master attract partita dopo verrebbe subito smutata.
  char userPlayIntentNow = input.startOrCoinPressed();
  if (menu.isAttractPlay() && userPlayIntentNow)
    menu.setAttractPlay(false);
  // MASTER_ATTRACT_MUTE_AUDIO: mute durante la demo del master attract e,
  // per le macchine che lo sanno rilevare (audioAttractMute()), anche durante
  // l'attract mode interno del gioco stesso (es. pbaction bit0 di 0xC000).
  audio.mute(!isMenu && (menu.isAttractPlay() || currentMachine->audioAttractMute()));
#endif
  if(isMenu) {
    menu.handle();
  }
...
#endif
  const signed char machineType = currentMachine->machineType();
  const unsigned short renderWidth = (!isMenu && (machineType == MCH_SCREGG || machineType == MCH_QIX || machineType == MCH_KANGAROO)) ? 240 : 224;
...
void renderRow(short row, bool isMenu) {
  if(isMenu) {
    ...
    const signed char machineType = currentMachine->machineType();
    const unsigned short width = (machineType == MCH_SCREGG || machineType == MCH_QIX || machineType == MCH_KANGAROO) ? 240 : 224;
