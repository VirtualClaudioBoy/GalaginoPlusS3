...
bool Menu::isAttractPlay() {
  return attractPlay;
}

void Menu::setAttractPlay(bool on) {
  attractPlay = on;
}
...
void Menu::attract_resetTimer() {
  ...
  // NB: NON toccare attractPlay qui. Un input generico (joystick up/down,
  // extra/volume) mentre gira una demo del master attract NON deve riattivare
  // l'audio: con MASTER_ATTRACT_MUTE_AUDIO definita la demo resta muta finche'
  // non viene avviata una partita reale (COIN/START -> main.cpp) o si torna
  // al menu (show_menu).
}
...
void Menu::handle() {
  ...
  if(((keymask & BUTTON_FIRE) && !(last_mask & BUTTON_FIRE)) || ((keymask & BUTTON_START) && !(last_mask & BUTTON_START))) {
    ...
    attractPlay = false;   // selezione esplicita dell'utente
  ...
  if(master_attract_timeout && (millis() - master_attract_timeout > MASTER_ATTRACT_MENU_TIMEOUT)) {
    ...
    machineIndex = menu_sel;
    attractPlay = true;   // avviato dal master attract, non da un input reale
