...
void Audio::start(machineBase *machineBase) {
  ...
  updateVolumeGain();   // sincronizza il gain col volumeSetting corrente
  ...
  else if (machineType == MCH_BTIME)      { AY = 2; AY_INC = 8; AY_VOL = 7;  }
  ...
  else if (machineType == MCH_KANGAROO)  { AY = 1; AY_INC = 6; AY_VOL = 11; }
  ...
  for (int i = 0; i < 3; i++) {
    vg_sample_pos[i] = vg_sample_len[i] = 0;
    vg_sample_ptr[i] = nullptr;
    vg_adpcm_predictor[i] = vg_adpcm_step[i] = vg_sample_repeat[i] = 0;
    vg_sample_divider[i] = 1;
  }
...
void Audio::updateVolumeGain() {
  // volumeSetting = 1 → 100%, 10 → ~29%, 20 → ~6%, 30 → ~1.6%
  volumeGain = powf(0.5f, (volumeSetting - 1) / 5.0f);
}
...
void Audio::transmit() {
  ...
  do {
    ...
    // Duplica il canale mono su L/R per MAX98357A
//    for (int i = 0; i < 64; i++) {
//      snd_buffer[(i * 2) + 1] = snd_buffer[i * 2];  // copia su canale sinistro
//    }
    ...
    else if(machineType == MCH_QIX)
      qix_render_buffer();
  } while(bytesOut);
}

void Audio::qix_render_buffer(void) {
  for(int i=0;i<64;i++) valueToBuffer(i,currentMachine->renderDacSample());
}
...
void Audio::vanguard_render_buffer(void) {
  ...
  if (currentMachine->soundregs[5] != vg_port_sequence[0]) {
    ...
    if (!is_fantasy && (p0 & 0x20) && !(vg_last_port0 & 0x20)) {
      ...
      vg_sample_len[0] = currentMachine->vanguardSampleLength(0);
      vg_sample_pos[0] = 0;
      vg_sample_divider[0] = currentMachine->vanguardSampleDivider(0);
      vg_sample_repeat[0] = 0;
    } else if (!(p0 & 0x20) && (vg_last_port0 & 0x20)) {
      vg_sample_len[0] = 0;
    }
    if (!is_fantasy && (p0 & 0x80) && !(vg_last_port0 & 0x80)) {
      vg_sample_ptr[1] = currentMachine->vanguardSample(1);
      vg_sample_len[1] = currentMachine->vanguardSampleLength(1);
      vg_sample_pos[1] = 0;
      vg_sample_divider[1] = currentMachine->vanguardSampleDivider(1);
      vg_sample_repeat[1] = 0;
    }
    vg_last_port0 = p0;
  }
  ...
  if (speech_sequence != vg_speech_sequence) {
    ...
    if (phrase == 0xff) {
      ...
      vg_sample_len[2] = currentMachine->vanguardSampleLength(2 + phrase);
      vg_sample_pos[2] = 0;
      vg_sample_divider[2] = currentMachine->vanguardSampleDivider(2 + phrase);
      vg_sample_repeat[2] = 0;
    }
  }
  ...
  for (int i = 0; i < 64; i++) {
    ...
    for (int ch = 0; ch < (is_fantasy ? 3 : 2); ch++) {
      ...
      unsigned short base = ch == 2 ? (0x1000 | ((p3 & 0x70) << 4))
        : ((ch ? 0x0800 : 0) | ((ch ? p1 : p0) & 7) * 0x100);
      uint8_t note = currentMachine->vanguardSoundRom(base + vg_offset[ch]);
      if (note == 0xff) continue;
      uint32_t step = 939349UL / (256 - note);
      ...
      sample += ((previous * (int)(0x10000 - fraction) + current * (int)fraction) >> 16) * 16;
    ...
    for (int voice = 0; voice < 3; voice++) {
      if (!vg_sample_ptr[voice] || vg_sample_pos[voice] >= vg_sample_len[voice]) continue;
      if (!vg_sample_repeat[voice]) {
        if (!vg_sample_pos[voice]) {
          const uint8_t *encoded = (const uint8_t *)vg_sample_ptr[voice];
          vg_adpcm_predictor[voice] = (int16_t)(encoded[0] | (encoded[1] << 8));
          vg_adpcm_step[voice] = 0;
        } else {
        ...
        vg_sample_pos[voice]++;
        vg_sample_repeat[voice] = vg_sample_divider[voice];
      }
      sample += vg_adpcm_predictor[voice] >> 8;
      vg_sample_repeat[voice]--;
    }
...
void Audio::ay_render_buffer(void) {
  ...
  for(int i = 0; i < 64; i++) {
    ...
    int ay_step = AY_INC;
    if (machineType == MCH_KANGAROO) {
      // AY clock 1.25 MHz, divider 8, output 24 kHz: 1250/192 ticks/sample.
      kangaroo_ay_phase += 1250;
      ay_step = kangaroo_ay_phase / 192;
      kangaroo_ay_phase %= 192;
    }

    for(char ay = 0; ay < AY; ay++) {
      // --- LOGICA INVILUPPO: Esegui un passo di emulazione ---
      if (!ay_envelope_holding[ay] && ay_envelope_period[ay] > 0) {
        ay_envelope_counter[ay] += ay_step;
...
void Audio::sn76489_render_buffer(void) {
  ...
  for (int chip = 0; chip < NUM_SN_CHIPS; chip++) {
    ...
    // Circus Charlie: mix del DAC 8 bit (effetti) sopra i due SN76489A.
    // Come i drums di Gyruss: pop dal ring riempito in run_frame sul core
    // emulazione (MAI steppare CPU qui) + DC blocker per l'offset di riposo.
    if (machineType == MCH_CIRCUSC) {
      static int dc = 0;
      int s = currentMachine->renderDrumSample();  // gia' scalato (volume in circusc.h)
      dc += (s - dc) >> 7;
      sample += s - dc;
    }
...
void Audio::namco_render_buffer(void) {
  ...
  for(int i = 0; i < 64; i++) {
    ...
    if(machineType == MCH_GALAGA) {
      ...
      // campioni gia' a 24kHz = sample rate galagino: 1 byte per campione
      // (piena velocita', stile gaplus::snd_bang — lo schema galaga sopra
      // avanza ogni 2 campioni perche' il suo WAV storico e' a 12kHz)
...
void Audio::valueToBuffer(int index, short value) {
    ...
    // Espandi da +/-512 a +/-32767
    int32_t sample = value * 64;

    // Volume digitale: il gain e' precalcolato in updateVolumeGain() solo quando
    // volumeSetting cambia (curve esponenziale, ogni 5 step il volume dimezza:
    // setting 1 → 100%, 10 → ~29%, 20 → ~6%, 30 → ~1.6%).
    sample = int32_t(sample * volumeGain);

    // Clamping
    if (sample > 32767) sample = 32767;
    if (sample < -32768) sample = -32768;

#ifdef SND_LEFT_CHANNEL
    // MAX98357A: amplificatore DIGITALE I2S. Richiede dati signed 16-bit
    // (2's complement), NON unsigned con offset. Il formato unsigned
    // (0x8000 + sample) era per il DAC analogico PAM8302A e produce
    // un segnale distorto/forte sul MAX98357A indipendentemente dal gain.
    snd_buffer[index] = (uint16_t)sample;
#else
    // DAC analogico (PAM8302A): unsigned con offset + workaround byte order
    uint16_t out = uint16_t(sample + 32768);
    snd_buffer[index ^ 1] = out;
#endif
}
